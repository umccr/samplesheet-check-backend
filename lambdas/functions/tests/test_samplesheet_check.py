...
class TestSamplesheetCheckUnitTestCase(TestCase):
  ...
  def test_run_check_no_error(self):
    assert run_sample_sheet_content_check("SampleSheetClass") == None, "Expected with no error return"
  ...
  def test_run_check_SimilarIndexError(self):
    assert run_sample_sheet_content_check("SampleSheetClass") == "Found at least two indexes that were too similar to each other", "Expected error"

  @mock.patch('samplesheet_check.get_years_from_samplesheet',  mock.MagicMock(return_value=set((2019,2020,2021))))
  @mock.patch('samplesheet_check.check_samplesheet_header_metadata',  mock.MagicMock(side_effect=SampleSheetHeaderError))
  @mock.patch('samplesheet_check.check_sample_sheet_for_index_clashes',  mock.MagicMock(return_value=""))
  @mock.patch('samplesheet_check.set_meta_data_by_library_id',  mock.MagicMock(return_value=""))
  @mock.patch('samplesheet_check.check_metadata_correspondence',  mock.MagicMock(return_value=""))
  @mock.patch('samplesheet_check.check_global_override_cycles',  mock.MagicMock(return_value=""))
  ...
  def test_run_check_SampleSheetHeaderError(self):
    assert run_sample_sheet_content_check("SampleSheetClass") == "Samplesheet header did not have the appropriate attributes", "Expected error"
  ...
  def test_run_check_GetMetaDataError(self):
    ...
    assert run_sample_sheet_check_with_metadata("SampleSheetClass","JWT-TOKEN") == "Unable to get metadata", "Expected error"
  ...
  def test_run_check_MetaDataError(self):
    ...
    assert run_sample_sheet_check_with_metadata("SampleSheetClass","JWT-TOKEN") == "Metadata could not be extracted", "Expected error"
  ...
  def test_run_check_globalOverrideCyclesError(self):
    ...
    assert run_sample_sheet_check_with_metadata("SampleSheetClass","JWT-TOKEN") == "Override cycles check failed", "Expected error"
